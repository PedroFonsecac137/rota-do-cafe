...
game.addEventListener('hud',e=>{$('#score').textContent=Math.round(e.detail.score).toLocaleString('pt-BR');const combo=e.detail.combo>=5?2:1,capri=e.detail.bonusTime>0?2:1;$('#combo').textContent=capri>1?`x${combo*capri} 🎒`:e.detail.combo>=5?'x2':`${e.detail.combo}/5`;$('#time').textContent=Math.ceil(e.detail.time);});
...
game.addEventListener('end',e=>{stopMusic();shell.classList.remove('playing','final-rush');$('#hud').hidden=true;$('#mobileControls').hidden=true;$('#finalScore').textContent=Math.round(e.detail.score).toLocaleString('pt-BR');$('#resultTitle').textContent=e.detail.score>3500?'Colheita incrível!':e.detail.score>1800?'Boa colheita!':'Continue treinando!';$('#matchStats').innerHTML=`<div><strong>${e.detail.collected}</strong><small>cafés +100</small></div><div><strong>${e.detail.basePoints.toLocaleString('pt-BR')}</strong><small>pontos da coleta</small></div><div><strong>+${e.detail.bonusPoints.toLocaleString('pt-BR')}</strong><small>bônus de combo</small></div>`;render(addScore(player,phone,Math.round(e.detail.score)),e.detail.score);tone(180,.45,'triangle',.045);$('#endScreen').classList.add('active');});
