...
export class Game extends EventTarget{
 ...
 start(){Object.assign(this,{mode:'playing',score:0,combo:0,maxCombo:0,collected:0,mistakes:0,time:DURATION,elapsed:0,speed:112,next:1,y:0,vy:0,land:0,slide:0,slideHold:false,inv:0,rush:false,birdScheduled:false,dying:false,deathTimer:0,deathType:'',runStep:-1,objects:[],particles:[]});this.emit('hud');}
 ...
 update(dt){this.elapsed+=dt;if(this.dying){this.deathTimer+=dt;for(const o of this.objects)o.x-=this.speed*dt*.18;for(const p of this.particles){p.x+=p.vx*dt;p.y+=p.vy*dt;p.vy+=130*dt;p.life-=dt;}this.particles=this.particles.filter(p=>p.life>0);if(this.deathTimer>(this.deathType==='fall'?1.05:.82))this.finish();return;}this.time=Math.max(0,DURATION-this.elapsed);this.speed=112+this.elapsed*1.55;this.land=Math.max(0,this.land-dt);this.slide=this.slideHold&&this.y<2?1:Math.max(0,this.slide-dt);if(this.slide)this.rollTime+=dt;this.inv=Math.max(0,this.inv-dt);const wasY=this.y;if(this.vy||this.y){this.vy-=520*dt;this.y=Math.max(0,this.y+this.vy*dt);if(!this.y){this.vy=0;if(wasY>0){this.land=.16;this.dust(96,G-2,12);}}}if(!this.birdScheduled&&this.elapsed>3.2&&!this.objects.some(o=>o.x>335)){this.birdScheduled=true;this.objects.push({type:'bird',x:500,y:146,w:40,h:22,good:false,points:-100});}this.next-=dt;if(this.next<=0){this.spawn();this.next=Math.max(.78,1.24-this.elapsed*.01)+Math.random()*.32;}for(const o of this.objects)o.x-=this.speed*dt;
  if(!this.y&&!this.slide){const runFps=10+Math.min(4,(this.speed-112)/28),step=Math.floor(this.elapsed*runFps)%6;if(step!==this.runStep){this.runStep=step;if(step===1||step===4)this.dust(91,G-1,5);}}
 ...
 hit(o){o.hit=true;if(o.good){this.combo++;this.maxCombo=Math.max(this.maxCombo,this.combo);this.collected++;const m=1+Math.min(4,Math.floor(this.combo/5));this.score+=o.points*m;this.emit('feedback',{good:true,text:`+${o.points*m} CAFÉ MADURO`});for(let i=0;i<8;i++)this.particles.push({x:o.x+8,y:o.y+8,vx:rnd(-70,70),vy:rnd(-105,-35),life:rnd(.4,.7),c:C.yellow});return;}this.combo=0;this.mistakes++;this.dying=true;this.deathTimer=0;this.deathType=o.type==='hole'?'fall':'hit';this.slide=0;this.slideHold=false;this.vy=0;this.emit('feedback',{good:false,text:o.type==='hole'?'CAIU NO BURACO!':'BATEU! FIM DE JOGO'});for(let i=0;i<12;i++)this.particles.push({x:o.x+8,y:o.y+8,vx:rnd(-80,80),vy:rnd(-115,-40),life:rnd(.45,.8),c:C.red});}
 finish(){if(this.mode==='ended')return;this.mode='ended';this.emit('end',{collected:this.collected,mistakes:this.mistakes,maxCombo:this.maxCombo,golden:0,accuracy:Math.round(this.collected/Math.max(1,this.collected+this.mistakes)*100)});}
 ...
 playerPolished(){
  ...
  if(this.dying&&this.deathType==='fall'&&this.specialActions.complete){
   ...
   isRun=true;
   const fps=10+Math.min(4,(this.speed-112)/28),cycle=this.elapsed*fps,frame=Math.floor(cycle)%6,mix=cycle%1,smooth=mix*mix*(3-2*mix),next=(frame+1)%6;
