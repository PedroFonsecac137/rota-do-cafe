...
export class Game extends EventTarget{
 ...
 loop(n){const dt=Math.min(.034,(n-this.last)/1000);this.last=n;this.t+=dt;if(this.mode==='playing'){this.update(dt);if(this.vy===0)this.jumps=0;if(this.elapsed>=this.nextStageAt){this.spawn();this.nextStageAt+=this.stageStep<12?1.1:.92;}this.speed*=1.18;}this.draw();requestAnimationFrame(x=>this.loop(x));}
 ...
 spawn(){
  ...
  pattern.forEach((type,i)=>{
   ...
   this.objects.push({type,x:500+i*66,y,w,h,good:false,points:type==='borer'||type==='bird'||type==='bees'?-100:-70});
  });
 }
